...
class BGHead2(FCNHead):

    def __init__(self, boundary_threshold=0.1, **kwargs):
        super(BGHead2, self).__init__(**kwargs)
        ...
        self.register_buffer(
            'output_laplacian_kernel',
            torch.tensor([[-1, -1, -1, -1, 8, -1, -1, -1, -1],[-1, -1, -1, -1, 8, -1, -1, -1, -1]],
                         dtype=torch.float32,
                         requires_grad=False).reshape((1, 2, 3, 3)))
    ...
    def losses(self, seg_logit, seg_label):
        ...
        """Compute outputs Boundary"""
        # 不设置阈值和clamp，否则无法反向传播
        seg_logit = seg_logit.to(self.output_laplacian_kernel)

        boundary_outputs = F.conv2d(
            seg_logit, self.output_laplacian_kernel, padding=1)
        # boundary_outputs = boundary_outputs.clamp(min=0)
        # boundary_outputs[boundary_outputs > self.boundary_threshold] = 1
        # boundary_outputs[boundary_outputs <= self.boundary_threshold] = 0

        boundary_outputs_x2 = F.conv2d(
            seg_logit, self.output_laplacian_kernel, stride=2, padding=1)
        # boundary_outputs_x2 = boundary_outputs_x2.clamp(min=0)
        boundary_outputs_x2_up = F.interpolate(
            boundary_outputs_x2, boundary_outputs.shape[2:], mode='nearest')
        # boundary_outputs_x2_up[
        #     boundary_outputs_x2_up > self.boundary_threshold] = 1
        # boundary_outputs_x2_up[
        #     boundary_outputs_x2_up <= self.boundary_threshold] = 0

        boundary_outputs_x4 = F.conv2d(
            seg_logit, self.output_laplacian_kernel, stride=4, padding=1)
        # boundary_outputs_x4 = boundary_outputs_x4.clamp(min=0)
        boundary_outputs_x4_up = F.interpolate(
            boundary_outputs_x4, boundary_outputs.shape[2:], mode='nearest')
        # boundary_outputs_x4_up[
        #     boundary_outputs_x4_up > self.boundary_threshold] = 1
        # boundary_outputs_x4_up[
        #     boundary_outputs_x4_up <= self.boundary_threshold] = 0

        boundary_outputs_pyramids = torch.stack(
            (boundary_outputs, boundary_outputs_x2_up, boundary_outputs_x4_up), dim=1)
        boundary_outputs_pyramids = boundary_outputs_pyramids.squeeze(2)
        boundary_outputs_pyramid = F.conv2d(boundary_outputs_pyramids, self.fusion_kernel)
        # boundary_outputs_pyramid[
        #     boundary_outputs_pyramid > self.boundary_threshold] = 1
        # boundary_outputs_pyramid[
        #     boundary_outputs_pyramid <= self.boundary_threshold] = 0

        loss = super(BGHead2, self).losses(boundary_outputs_pyramid, boundary_targets_pyramid.long())
